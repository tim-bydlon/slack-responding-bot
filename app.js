		...
		if (results.rows) {
			...
			else if (slackResponse)
			{
				...
				if (responseObj.text.includes("{Random_Assign}")) {
					const channel_contact_ids = await pool.query('SELECT slack_id__c FROM salesforce.Contact__c WHERE $1 = ANY(channels__c);', [channel.id]);
					const randomIndex = Math.floor(Math.random() * channel_contact_ids.rows.length);
					const assigned_contact_id = channel_contact_ids.rows[randomIndex].slack_id__c;
